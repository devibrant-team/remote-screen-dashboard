...
import SelectScreenModal from "./Models/SelectScreenModal";
...
type Props = {
  ...
  onOpenScheduledScreens?: () => void; // (unused now; handled locally)
  onCreated?: (args: { serverId: number }) => void;
  onDeleteReserved?: (id: number | string) => void | Promise<void>;
};
...
const ScheduleAssignSidebar: React.FC<Props> = ({
  ...
  // local modal state for Add/View
...
const normalizedScheduleId = useMemo<number | null>(() => {
  ...
  const updateTimes = (
    patch: Partial<
      Pick<ScheduleReduxItem, "startDate" | "startTime" | "endDate" | "endTime">
    >
  ) => {
    if (mode === "editReserved") {
      setDraft((prev) => (prev ? { ...prev, ...patch } : prev));
    } else {
      const src = workingItem as ScheduleReduxItem;
      dispatch(
        setItemTimes({
          id: src.id,
          startDate: patch.startDate ?? src.startDate,
          startTime: patch.startTime ?? src.startTime,
          endDate: patch.endDate ?? src.endDate,
          endTime: patch.endTime ?? src.endTime,
        })
      );
    }
  };
      ...
      setDraft((prev) =>
        prev
          ? {
              ...prev,
              playlistId,
              title: title?.trim() ? title : `Playlist #${playlistId}`,
            }
          : prev
      );
      return;
...
const handleSubmit = async () => {
  ...
  return (
    <>
      <aside
        lang="en-GB"
        className="fixed right-0 top-0 z-40 h-full w-[360px] border-l border-gray-200 bg-white shadow-xl"
      >
        {/* Header */}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    value={toDateInput(workingItem.startDate)}
                    onChange={(e) =>
                      updateTimes({ startDate: fromDateInput(e.target.value) })
                    }
                    disabled={busy}
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="date"
                    className="w-full rounded-md border border-gray-300 bg-white px-2 py-1.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-red-400"
                    value={toDateInput(workingItem.endDate)}
                    onChange={(e) =>
                      updateTimes({ endDate: fromDateInput(e.target.value) })
                    }
                    disabled={busy}
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <TimeFieldWithPicker
                    label="Start time"
                    value={toHMS(workingItem.startTime)}
                    onChange={(v) => updateTimes({ startTime: v })}
                    minuteStep={5}
                    secondStep={5}
                    disabled={busy}
                    compact
                ...
                <div className="space-y-1.5">
                  <TimeFieldWithPicker
                    label="End time"
                    value={toHMS(workingItem.endTime)}
                    onChange={(v) => updateTimes({ endTime: v })}
                    minuteStep={5}
                    secondStep={5}
                    disabled={busy}
                    compact
                              ...
                              if (mode === "editReserved") {
                                setDraft((prev) => {
                                  ...
                                  if (e.target.checked) {
                                    ...
                                    return {
                                      ...prev,
                                      screens: Array.from(nextIds).map(
                                        (id) => ({ screenId: id })
                                      ),
                                    };
                                  } else {
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                ...
                className={
                  ...
                  (busy ? "cursor-not-allowed opacity-75" : "hover:bg-red-600")
                }
                disabled={busy}
              >
                {busy
      ...
      {/* Target selection modal (above sidebar) */}
      <SelectScreenModal
        open={targetsOpen}
        onClose={() => setTargetsOpen(false)}
        onConfirmNavigate={() => {
          // You can navigate to /schedule or just close; your modal already posts and updates store.
