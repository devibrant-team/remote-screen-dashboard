...
const NormalSlider = () => {
  ...
  const playlistSlides = useSelector((state: RootState) => state.playlist.slides);
  const selectedIndex = useSelector((state: RootState) => state.playlist.selectedSlideIndex);
  const normalSlide = useSelector((state: RootState) => state.normalplaylist);

  // Add current slide to playlist
  const handleAddSlide = () => {
    dispatch(addSlide(normalSlide));
  };

  // Auto-select the first slide by default
  useEffect(() => {
    if (playlistSlides.length > 0 && selectedIndex === null) {
      dispatch(setSelectedSlideIndex(0));
    }
  }, [playlistSlides, selectedIndex, dispatch]);

  // Logging for debugging
  useEffect(() => {
    console.log("[Test] Current playlist slides:", playlistSlides);
  }, [playlistSlides]);
